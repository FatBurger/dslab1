...
import proxy.commands.ClientDownloadCommand;
import proxy.commands.ClientExitCommand;
import proxy.commands.ClientListCommand;
...
public class TcpConnectionHandler implements Runnable
{
   ...
   private void RegisterCommands()
   {
      ...
      ClientUnknownCommand unknownCommand = new ClientUnknownCommand(
               outgoingProtocol);
      messageCommandHandler.RegisterDefaultCommand(unknownCommand);

      // register the login command
      ClientLoginCommand loginCommand = new ClientLoginCommand(userManager,
               outgoingProtocol, connection);
      messageCommandHandler.RegisterCommand(loginCommand.getIdentifier(),
               loginCommand);

      // register the credits command
      ClientCreditsCommand creditsCommand = new ClientCreditsCommand(
               userManager, outgoingProtocol, connection);
      messageCommandHandler.RegisterCommand(creditsCommand.getIdentifier(),
               creditsCommand);

      // register the buy command
      ClientBuyCommand buyCommand = new ClientBuyCommand(userManager,
               outgoingProtocol, connection);
      messageCommandHandler.RegisterCommand(buyCommand.getIdentifier(),
               buyCommand);

      // register the list command
      ClientListCommand listCommand = new ClientListCommand(userManager,
               outgoingProtocol, serverManager, connection);
      messageCommandHandler.RegisterCommand(listCommand.getIdentifier(),
               listCommand);

      // register the download command
      ClientDownloadCommand downloadCommand = new ClientDownloadCommand(
               userManager, outgoingProtocol, serverManager, connection);
      messageCommandHandler.RegisterCommand(downloadCommand.getIdentifier(),
               downloadCommand);

      // register the exit command
      ClientExitCommand exitCommand = new ClientExitCommand(userManager,
               outgoingProtocol, connection);
      messageCommandHandler.RegisterCommand(exitCommand.getIdentifier(),
               exitCommand);
   }
